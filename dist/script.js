...
function () {
  function Customizator() {
    ...
    }); //вешаем обработчик событий на input изменение цвета фона

    this.clear.addEventListener('click', function () {
      return _this.reset();
    }); //вешаем обработчик событий на кнопку сброса всех параметров
  }
  ...
  _createClass(Customizator, [{
    ...
    value: function onScaleChange(e) {
      ...
      //создаем метод для кнопок масштаба
      var body = document.querySelector('body');

      if (e) {
        //проверяем есть ли значение value у кнопок
      ...
      var recursy = function recursy(element) {
        //создаем функцию для изменения масштаба
        element.childNodes.forEach(function (node) {
          //перебираем каждый элементу заданного блока (будем передавать body)
          if (node.nodeName === '#text' && node.nodeValue.replace(/\s+/g, '').length > 0) {
            //проверяем, если Нода текстовая и значение ноды(с убранными пробелами) больше нуля
            if (!node.parentNode.getAttribute('data-fz')) {
              //если нет data аттрибута - т.е. не задавали его с помщью изменения масштаба 
              var value = window.getComputedStyle(node.parentNode, null).fontSize; //создаем переменную которая берет все св-ва font-size у родительского нода
              ...
              node.parentNode.style.fontSize = node.parentNode.getAttribute('data-fz') * _this2.scale + 'px'; //родительскому элементу присваивается inline стиль(исходный шрифт*масштаб заданный value у кнопки)
            } else {
              //иначе
              node.parentNode.style.fontSize = node.parentNode.getAttribute('data-fz') * _this2.scale + 'px'; //если масштаб уже менялся то не меняем масштаб. Присваиваем inline стиль(исходный шрифт*масштаб заданный value у кнопки(1х))
            }
          } else {
            recursy(node);
          }
    ...
    value: function setBgColor() {
      //метод для применения значений цвета из localStorage
      var body = document.querySelector('body');
